...
export const MMTuningSchema = z.object({
  op: z.enum(['status', 'top', 'replay', 'selftest', 'rebuild_start', 'rebuild_status', 'clip_motion']).describe('Read-only tuning read: live telemetry snapshot, top-N cost ranking from a trace file, deterministic replay verify, or native search self-test. rebuild_start queues a full DB rebuild (minutes, fire-and-forget); rebuild_status reads its progress marker. clip_motion audits per-clip baked motion (needs one play tick this session; reuses the preset field as a name filter).'),
...
export const MMApplyPresetSchema = z.object({
  preset: z.enum(['baseline', 'pose', 'turn']).describe('Weight preset: trajectory-heavy baseline, pose-heavy precision, or turn-heavy. Live scene weights, no rebake. Marks the scene edited.'),
