...
import {
  EnvironmentProbeBakeSchema, FoliageGetSummarySchema, LightingBakeSchema, LightingGetStatusSchema, LightingValidateSchema,
  NavigationBuildSchema, NavigationGetStatusSchema, NavigationQueryPathSchema, NavigationValidateAgentsSchema,
  PhysicsFindOverlapsSchema, PhysicsGetLayerMatrixSchema, PhysicsRaycastSchema, PhysicsValidateCollidersSchema, TerrainGetSummarySchema,
  handleEnvironmentProbeBake, handleFoliageGetSummary, handleLightingBake, handleLightingGetStatus, handleLightingValidate,
  handleNavigationBuild, handleNavigationGetStatus, handleNavigationQueryPath, handleNavigationValidateAgents,
  handlePhysicsFindOverlaps, handlePhysicsGetLayerMatrix, handlePhysicsRaycast, handlePhysicsValidateColliders, handleTerrainGetSummary,
} from './domainLive.js';
...
const INPUT_SCHEMAS: Record<string, z.ZodTypeAny> = {
  ...
  physics_validate_colliders: PhysicsValidateCollidersSchema,
  physics_raycast: PhysicsRaycastSchema,
  physics_get_layer_matrix: PhysicsGetLayerMatrixSchema,
  physics_find_overlaps: PhysicsFindOverlapsSchema,
  navigation_build: NavigationBuildSchema,
  navigation_get_status: NavigationGetStatusSchema,
  navigation_validate_agents: NavigationValidateAgentsSchema,
  navigation_query_path: NavigationQueryPathSchema,
  lighting_bake: LightingBakeSchema,
  lighting_get_status: LightingGetStatusSchema,
  lighting_validate: LightingValidateSchema,
  environment_probe_bake: EnvironmentProbeBakeSchema,
  terrain_get_summary: TerrainGetSummarySchema,
  foliage_get_summary: FoliageGetSummarySchema,
  ...
  graph_remove_node: GraphRemoveNodeSchema,
...
export function buildToolRegistry(ctx: ProjectMeta): ToolDefinition[] {
  const tools: Array<Omit<ToolDefinition, 'zodInputSchema' | 'outputSchema' | 'annotations'>> = [
    ...
    { name: 'physics_validate_colliders', description: 'Validates bounded public collider state in loaded scenes without changing physics.', inputSchema: zodToJsonSchema(PhysicsValidateCollidersSchema), handler: (a, c) => handlePhysicsValidateColliders(a as Parameters<typeof handlePhysicsValidateColliders>[0], c) },
    ...
    { name: 'navigation_build', description: 'Stable unsupported capability: Flax 1.12 navmesh building lacks a reviewed cancellation, undo, and bridge-owned completion contract.', inputSchema: zodToJsonSchema(NavigationBuildSchema), handler: (a, c) => handleNavigationBuild(a as Parameters<typeof handleNavigationBuild>[0], c) },
    ...
    { name: 'lighting_bake', description: 'Stable unsupported capability: Flax 1.12 lightmap baking has no reviewed bridge-owned lifecycle/cancel contract.', inputSchema: zodToJsonSchema(LightingBakeSchema), handler: (a, c) => handleLightingBake(a as Parameters<typeof handleLightingBake>[0], c) },
    ...
    { name: 'environment_probe_bake', description: 'Stable unsupported capability: async probe baking lacks a reviewed bridge-owned lifecycle/cancel contract.', inputSchema: zodToJsonSchema(EnvironmentProbeBakeSchema), handler: (a, c) => handleEnvironmentProbeBake(a as Parameters<typeof handleEnvironmentProbeBake>[0], c) },
    { name: 'terrain_get_summary', description: 'Lists bounded read-only metadata for loaded Terrain actors; terrain editing remains unavailable.', inputSchema: zodToJsonSchema(TerrainGetSummarySchema), handler: (a, c) => handleTerrainGetSummary(a as Parameters<typeof handleTerrainGetSummary>[0], c) },
    { name: 'foliage_get_summary', description: 'Lists bounded read-only metadata for loaded Foliage actors; painting and instance edits remain unavailable.', inputSchema: zodToJsonSchema(FoliageGetSummarySchema), handler: (a, c) => handleFoliageGetSummary(a as Parameters<typeof handleFoliageGetSummary>[0], c) },
    {
    ...
    },
    {
      ...
      description: 'Deletes one node from a Visject graph root context via the window save path (engine-protected NoRemove nodes and sole-root-node refused). Bounded Phase 5 macro; dry-run by default; removal is window-undoable but saving cannot be undone. Requires bridge v18.',
      inputSchema: zodToJsonSchema(GraphRemoveNodeSchema),
      handler: (a, c) => handleGraphRemoveNode(a as Parameters<typeof handleGraphRemoveNode>[0], c),
    },
    {
      name: 'graph_disconnect',
      description: 'Breaks one output-to-input wire between two boxes in a Visject graph root context via the window save path. Bounded Phase 5 macro; dry-run by default; the break pushes no undo action (graph.undo cannot restore it) and saving cannot be undone. Requires bridge v18.',
      inputSchema: zodToJsonSchema(GraphDisconnectSchema),
      handler: (a, c) => handleGraphDisconnect(a as Parameters<typeof handleGraphDisconnect>[0], c),
    },
    {
      name: 'graph_set_node_values',
      description: 'Sets value slots on one Visject graph root node via the window save path (type-coerced per slot, asset refs validated). Bounded Phase 6 tool; dry-run by default; value writes are window-undoable but saving cannot be undone. Requires bridge v20.',
      inputSchema: zodToJsonSchema(GraphSetNodeValuesSchema),
      handler: (a, c) => handleGraphSetNodeValues(a as Parameters<typeof handleGraphSetNodeValues>[0], c),
    },
    {
      name: 'graph_move_node',
    ...
    },
    {
      ...
      description: 'Assigns an animation clip to an AnimationGraph state by spawning a sampler from archetype defaults and wiring Pose to State Output. Bounded Phase 6 macro; dry-run by default; the wire pushes no undo action and saving cannot be undone. Requires bridge v20.',
      inputSchema: zodToJsonSchema(AnimgraphSetStateClipSchema),
      handler: (a, c) => handleAnimgraphSetStateClip(a as Parameters<typeof handleAnimgraphSetStateClip>[0], c),
    },
    {
