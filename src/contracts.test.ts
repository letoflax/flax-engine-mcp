...
test('release registry has a unique, version-aligned 106-tool contract', async () => {
...
test('permission profiles cover the registry and fail closed by default', () => {
  const names = tools.map(tool => tool.name);
  assert.equal(allowedToolNames(names, { profile: 'full', allowTools: [], denyTools: [], emergencyReadOnly: false }).length, 106);
